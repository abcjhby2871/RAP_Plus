...
class Detector():
    ...
    def detect_and_crop(self, image):
        ...
        # crop image and detected_region: include class name and box axis
        crops = []
        detected_regions = []
        
        # Function to check if a crop with the same bbox already exists in crops (or nearly the same)
        def is_duplicate_crop(bbox, tolerance=30):
            for detected_region in detected_regions:
                existing_bbox = detected_region['box']
                # Check if the difference between the bounding box coordinates is within the tolerance
                if all(abs(existing_bbox[i] - bbox[i]) <= tolerance for i in range(4)):
                    return True
            return False
        ...
        for i, (bbox, class_id) in enumerate(zip(bboxes, class_ids)):
            ...
            if class_name == "person":
                ...
                for face_location in face_locations:
                    ...
                    # Crop each face and append it to the crops list
                    face_crop = image.crop((left, top, right, bottom))
                    if not is_duplicate_crop([left, top, right, bottom]):
                        crops.append(face_crop)
                        detected_regions.append({
                            "class_name": "person",
                            "box": [left, top, right, bottom]
...
if __name__ == "__main__":
    ...
    for idx, (crop, detected_region) in enumerate(zip(crops, detected_regions)):
        print(2)
        save_path = f"crop_image/crop_{idx}.jpeg"  # 这里你可以根据需要自定义文件名
        crop.save(save_path)
        print(f"Saved cropped image to {save_path}")
